...
const Pcbuild = () => {
  ...
  const handleSavedPc = async (savepc, email) => {
    if (checkRequiredFields()) {
      ...
      if (savepc.length !== 0) {
        const res = await axiosSecure.post("/savedPc", data);
        toast("Created pc added", { autoClose: 1000 });
        if (res.data?.insertedId) {
        }
      }
    } else {
      ...
      <div ref={targetRef} className="max-w-screen-lg mx-auto mt-10">
        ...
        <table className="table border border-1  shadow-lg ">
          <tbody>
            ...
            {renderTableRow(
              "Cpu Cooler",
              selectedItems?.cpuCooler?.img || cpuCooler,
              selectedItems?.cpuCooler?.name || "Cpu Cooler",
              selectedItems?.cpuCooler?.price || "",
              "cpucoolers",
              selectedItems?.cpuCooler?.cartItemId
            )}
            {renderTableRow(
              "Motherboard",
              selectedItems?.motherboard?.img || motherboard,
